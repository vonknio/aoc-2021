...
-- Return (new grid, flash count, number of rounds that remained at the moment when all cells flashed at the same time)
flash_in_rounds :: [[Int]] -> Int -> ([[Int]], Int)
flash_in_rounds grid 0 = (grid, 0)
flash_in_rounds grid rounds =
  if count == length grid * length (grid !! 0)
    then (new_grid, rounds - 1)
  else 
    flash_in_rounds new_grid (rounds - 1)
  where
    (new_grid, count) = 
...
main = do
  ...
  case args of
    [fileName] -> do
      ...
      let max_rounds = 10000000
      let (_, res) = flash_in_rounds octops max_rounds
      print $ max_rounds - res
    _ -> do

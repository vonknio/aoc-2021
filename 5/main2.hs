...
generate_one_diagonal_axis :: Int -> Int -> [Int]
generate_one_diagonal_axis a b = if a <= b then [a..b] else [a,a-1..b]

generate_line :: [Int] -> [(Int, Int)]
generate_line (a:b:c:d:[])
  | a == c = map (\x -> (a, x)) [min b d..max b d]
  | b == d = map (\x -> (x, b)) [min a c..max a c]
  | otherwise = zip (generate_one_diagonal_axis a c) (generate_one_diagonal_axis b d)
